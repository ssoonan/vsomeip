...
public:
    service_sample(bool _use_static_routing) :
            ...
            offer_thread_(std::bind(&service_sample::run, this)) {
                boot_start = std::chrono::high_resolution_clock::now();
    }
    ...
    void on_state(vsomeip::state_type_e _state) {
        ...
        if (_state == vsomeip::state_type_e::ST_REGISTERED) {
            if (!is_registered_) {
                ...
                condition_.notify_one();
                offer();
                // boot_end = std::chrono::high_resolution_clock::now();
                // std::chrono::milliseconds duration = std::chrono::duration_cast<std::chrono::milliseconds>(boot_end - boot_start);
                // std::cout << "\nboot delay: " << std::dec << duration.count() << " milliseconds\n" << std::endl;
                // std::ofstream outputFile("boot_time.txt", std::ios::app);
    ...
    void on_message(const std::shared_ptr<vsomeip::message> &_request) {
        std::cout << "Received a message with Client/Session ["
          ...
          << " Length: " << std::dec << _request->get_payload()->get_length()
		  << std::endl;
        ...
        //its_response->set_payload(_request->get_payload());
...
private:
    ...
    std::chrono::high_resolution_clock::time_point  boot_start;
    std::chrono::high_resolution_clock::time_point  boot_end;
