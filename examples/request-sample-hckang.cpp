...
class client_sample {
public:
    client_sample(bool _use_tcp, bool _be_quiet, uint32_t _cycle, uint32_t _size, std::string _output)
        ...
        //   requester_(std::bind(&client_sample::request, this)) {
    }
    ...
    void on_state(vsomeip::state_type_e _state) {
        std::cout << "on_state" << std::endl;
        
        if (_state == vsomeip::state_type_e::ST_REGISTERED) {
            findservice_start = std::chrono::high_resolution_clock::now();
    ...
    void on_availability(vsomeip::service_t _service, vsomeip::instance_t _instance, bool _is_available) {
        ...
        std::cout
                ...
                << std::endl;
        // if(_is_available) {
        //     findservice_end = std::chrono::high_resolution_clock::now();
        //     std::chrono::milliseconds duration = std::chrono::duration_cast<std::chrono::milliseconds>(findservice_end - findservice_start);
        //     std::cout << "Findservice duration: " << std::dec << duration.count() << " milliseconds" << std::endl;
        //     std::ofstream outputFile("findservice_time.txt", std::ios::app);
        //     if (outputFile.is_open()) {
        //         // Append the measured duration to the file
    ...
    void on_message(const std::shared_ptr< vsomeip::message > &_response) {
        ...
        std::chrono::milliseconds duration = std::chrono::duration_cast<std::chrono::milliseconds>(recv_time - sent_time);
        
        std::cout << "Echo time: " << std::dec << duration.count() << " milliseconds" << std::endl;
        
        std::ofstream outputFile(output_ + "_echo_time_" + std::to_string(size_) + "bytes.txt", std::ios::app);
        if (outputFile.is_open()) {
            // Append the measured duration to the file
            outputFile << duration.count() << std::endl;
            outputFile.close();
        ...
        std::cout << "Received a response from Service ["
                ...
                << "]"
                << " Length: " << std::dec << _response->get_payload()->get_length()
                << std::endl;
        count_++;
        if(count_ == 5000) {
            stop();
    ...
    void run() {
        while (running_) {
            std::cout << "run" << std::endl;
            {
                ...
                if (is_available_) {
                    sent_time = std::chrono::high_resolution_clock::now();
                    app_->send(request_);
...
private:
    ...
    std::chrono::high_resolution_clock::time_point  findservice_start;
    std::chrono::high_resolution_clock::time_point  findservice_end;
...
int main(int argc, char **argv) {
    ...
    while (i < argc) {
        if (tcp_enable == argv[i]) {
            ...
            converter << argv[i];
            converter >> cycle;
        } else if (output_arg == argv[i] && i+1 < argc) {
            i++;
            std::stringstream converter;
            converter << argv[i];
    ...
    client_sample its_sample(use_tcp, be_quiet, cycle, size, output);
#ifndef VSOMEIP_ENABLE_SIGNAL_HANDLING
